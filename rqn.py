...
from rqn_net import SumNet, RQNet
...
class RQN:
	def __init__(self, args):
		...
		self.eval_rqn_net = RQNet(args)  # netowrk that adds up agents Q values 
		self.args = args
		...
		if self.args.cuda:
			...
			self.eval_sum_net.cuda()
			self.target_sum_net.cuda()
			# changed added this line
			self.eval_rqn_net.cuda()
		...
		if self.args.load_model:
		    if os.path.exists(self.model_dir + '/rnn_net_params.pkl'):
		        ...
		        path_sum = self.model_dir + '/sum_net_params.pkl'
		        self.eval_rnn.load_state_dict(torch.load(path_rnn))
		        self.eval_sum_net.load_state_dict(torch.load(path_sum))
		        print('Successfully load the model: {} and {}'.format(path_rnn, path_sum))
		    else:
		...
		self.target_sum_net.load_state_dict(self.eval_sum_net.state_dict())

		self.eval_parameters = list(self.eval_sum_net.parameters()) + list(self.eval_rnn.parameters())
		self.rqn_parameters = list(self.eval_rqn_net.parameters())
		if args.optimizer == "RMS":
		    self.optimizer = torch.optim.RMSprop(self.eval_parameters, lr=args.lr)
		    self.rqn_optimizer = torch.optim.RMSprop(self.rqn_parameters, lr=args.lr)
	...
	def learn(self, batch, max_episode_len, train_step, epsilon=None):  
		...
		q_evals = self.eval_rqn_net(q_evals)  # adds estimation factor to the q_values
		q_total_eval = self.eval_sum_net(q_evals)

		q_total_target = self.target_sum_net(q_targets)
		...
		self.rqn_optimizer.zero_grad()
		loss_rqn.backward()
		torch.nn.utils.clip_grad_norm_(self.rqn_parameters, self.args.grad_norm_clip)
		self.rqn_optimizer.step()
	...
	def save_model(self, train_step):
		...
		torch.save(self.eval_sum_net.state_dict(), self.model_dir + '/' + num + '_sum_net_params.pkl')

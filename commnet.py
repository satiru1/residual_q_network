...
class CommNet(nn.Module):
	def __init__(self, input_shape, args):
		...
		#self.cos_s = nn.CosineSimilarity(dim=1) # changed added this line
...
#		sim = self.cos_s(obs_encoding, h_in)  # gets similarity level
#		sim = sim.reshape(-1, 1).repeat(1, self.args.rnn_hidden_dim)  # (n_agents, rnn_hidden_dim)
...
'''
		for k in range(self.args.k):
			if k == 0:
				...
				# mult by sim
				#h = h * 0.8 * sim 
				sim = self.cos_s(obs_encoding, h)  # gets similarity level
				sim = sim.reshape(-1, 1).repeat(1, self.args.rnn_hidden_dim)  # (n_agents, rnn_hidden_dim)
				h = h * sim
				...
				#c = c.logsumexp(dim=-2, keepdim=True)  # changed: added this line
				c = c.sum(dim=-2)
				#changed commented below and added above line
				#c = c.mean(dim=-2)  # (episode_num * max_episode_len, n_agents, rnn_hidden_dim)
				h = h.reshape(-1, self.args.rnn_hidden_dim)
				c = c.reshape(-1, self.args.rnn_hidden_dim)
